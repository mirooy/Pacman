...
def constructBayesNet(gameState):
    ...
    var = [X_POS_VAR, Y_POS_VAR] + HOUSE_VARS + obsVars
    walls = bn.constructEmptyBayesNet(var, edges, variableDomainsDict)
    return walls, obsVars
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsF= bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
...
def get(gameState, obsPos):
    bottomLeftPos, topLeftPos, bottomRightPos, topRightPos = gameState.getPossibleHouses()
    d = 1000
    adj = None
    for house in gameState.getPossibleHouses():
        x = util.manhattanDistance(obsPos, house)
        if x < d:
            adj= house
            d = x
    
    if adj == bottomLeftPos:
        return BOTTOM_LEFT_VAL
    if adj == bottomRightPos:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    "*** YOUR CODE HERE ***"
    prob = 0
    likely = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)

    for x in likely.getAllPossibleAssignmentDicts():
        
        if likely.getProbability(x) > prob:
            likely_x = x
            prob = likely.getProbability(x)
    return likely_x
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        left = 0
        right = 0
        likely = inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        
        for x in likely.getAllPossibleAssignmentDicts():
            if x[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and x[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                right = likely.getProbability(x)
            elif x[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL and x[FOOD_HOUSE_VAR] == TOP_LEFT_VAL:
                left = likely.getProbability(x)

        left_ = (left * WON_GAME_REWARD )+ ((1  - left) * GHOST_COLLISION_REWARD)
        right_ =(right * WON_GAME_REWARD )+ ((1 - right) * GHOST_COLLISION_REWARD)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob, exploration in self.getExplorationProbsAndOutcomes(evidence):
            values = evidence.copy()
            values.update(exploration)
            one, two = self.computeEnterValues(values, enterEliminationOrder)
            expectedValue = expectedValue+ max(one,two) * prob
