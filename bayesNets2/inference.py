...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for x in eliminationOrder:
            current_factors, infer_factor = joinFactorsByVariable(current_factors, x)
            if len(infer_factor.unconditionedVariables()) > 1:
                eliminated_factor = eliminate(infer_factor, x)
                current_factors.append(eliminated_factor)
