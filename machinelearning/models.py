...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.h = 100
        self.w_1 = nn.Variable(1, self.h)
        self.w_2 = nn.Variable(self.h ,1)
        self.b_1 = nn.Variable(self.h)
        self.b_2 = nn.Variable(1)
    ...
    def run(self, x, y=None):
        ...
        """
        "*** YOUR CODE HERE ***"
        graph = nn.Graph([self.w_1, self.w_2, self.b_1, self.b_2])
        for i in range(20000):
            inpy = nn.Input(graph,y)
            inpx = nn.Input(graph, x)
            xm = nn.MatrixMultiply(graph, inpx, self.w_1)
            xmb = nn.MatrixVectorAdd(graph, xm, self.b_1)
            relu = nn.ReLU(graph, xmb)

            reluw = nn.MatrixMultiply(graph, relu, self.w_2)

            relub = nn.MatrixVectorAdd(graph, reluw, self.b_2)
        ...
        if y is not None:
            ...
            nn.SquareLoss(graph, relub, inpy)
            return graph
