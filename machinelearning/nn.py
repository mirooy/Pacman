...
class Graph(object):
    ...
    def get_inputs(self, node):
       ...
       i = self.graph.index(node)
        if i > 2:
            return self.graph[i-2:i]
        else: """
        "*** YOUR CODE HERE ***"
    ...
    def backprop(self):
        ...
        for node in self.get_nodes()[::-1]:
            newg = node.backward(self.get_inputs(node), self.get_gradient(node)) 
            
            parents = node.get_parents()
            if len(parents) == 1:
                self.gradients[parents[0]] = newg[0] + self.gradients[parents[0]]
            if len(parents) > 0:
                self.gradients[parents[0]] = newg[0] + self.gradients[parents[0]]
                self.gradients[parents[1]] = newg[1] + self.gradients[parents[1]]
    ...
    def step(self, step_size):
        ...
        for variable in self.variable:
            ...
            variable.data = (variable.data - self.get_gradient(variable)) * step_size
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        l.append(np.dot(g, y.T))
        l.append(np.dot(x.T, g))
        return l 
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        l.append(np.array(gradient))
        l.append(np.sum(gradient, axis=0))
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        l = list()
        x = np.array(inputs[0])
        l.append(np.where(x > 0 ,gradient, 0))
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        l.append(gradient * (a - b) / inputs[0].shape[0])
        l.append(gradient * (b - a) / inputs[0].shape[0])
