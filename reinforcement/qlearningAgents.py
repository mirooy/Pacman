...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(self.getLegalActions(state))==0:
          return None
        policy = self.computeValueFromQValues(state)
        bestaction = []
        for action in self.getLegalActions(state):
          if policy == self.getQValue(state,action):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        previous_Q = self.values[(state,action)]
        sample = reward + (self.discount * self.computeValueFromQValues(nextState))
        self.values[(state,action)] = ((1 - self.alpha)* previous_Q )+ (self.alpha * sample)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in features:
          total = total + features[feature] * self.getWeights()[feature]
        return total
